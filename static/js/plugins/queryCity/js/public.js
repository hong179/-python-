        ...
        function() {
            ...
            if (countyId == null || countyName == cityName)
            {
                if (currentClass == "proCityQuery")
                {
                    ...
                    $(".con .pname .list a").removeClass("current");
...
function viewHotCities() {
    $.each(cities,function(i, pname) {
        if (pname.hotCity) {
            $(".hotCity .list ul").append("<li><a><input type='button' style='background:none;border:0px;cursor: pointer;' onclick=hotCityAddrInput(\'" + pname.provinceId + "," + pname.id + "," + pname.name + "\') id='" + pname.id + "' value='" + pname.name + "'></a></li>");
        }
    });
}
...
$(".pname .pre a").bind('click',function() {
...
$(".pname .next a").bind('click',function() {
...
function viewCities(i) {
    ...
    $.each(cities,
        function(i, pname) {
            if (pname.provinceId == proId) {
                citys[j++] = pname;
            }
        });
    cityTotalPage = Math.ceil(citys.length / p_pageSize);
    $(".provinceCity").find(".tabs").find("a").removeClass("current");
    $(".provinceCity .tabs").find("#pname").addClass("current");
    $(".con .province .list a").removeClass("current");
    $(".con .province .list a[id='" + proId + "']").addClass("current");
    $(".provinceCity").find(".con").children().hide();
    $(".provinceCity").find(".con").find(".pname").show();
    cityPage(1);
}

function cityPage(page) {
    $(".pname .list ul li").remove();
    $(".cityAll .list ul li").remove();
    if (page == 1) {
        $(".pname .pre a").removeClass("can");
    } else {
        $(".pname .pre a").addClass("can");
    }
    var start;
    var end;
    if (page <= 1) {
        page = 1;
        $(".pname .pre a").removeClass("can");
        $(".pname .next a").addClass("can");
    }
    if (cityTotalPage == 1) {
        $(".pname .next a").removeClass("can");
        $(".pname .pre a").removeClass("can");
    }
    ...
    for (var i = start; i < end; i++) {
        ...
        $(".pname .list ul").append(li);
    }
    $(".pname .list #cityPage").remove();
    $(".pname .list").append("<label id='cityPage' style='display:none;'>" + page + "</label>");
}
...
function viewCounties(i) {
    ...
    $(".provinceCity .tabs").find("#county").addClass("current");
    $(".con .pname .list a").removeClass("current");
    $(".con .pname .list a[id='" + cityId + "']").addClass("current");
    $(".provinceCity").find(".con").children().hide();
    $(".provinceCity").find(".con").find(".county").show();
    countyPage(1);
}
...
function viewAllHotCities() {
    $.each(allCities,function(i, pname) {
        if (pname.hotCity) {
            $(".hotCityAll .list ul").append("<li><a><input type='button' style='background:none;border:0px;cursor: pointer;' onclick=hotCityAddrInputAll(\'" + pname.provinceId + "," + pname.id + "," + pname.name + "\') id='" + pname.id + "' value='" + pname.name + "'></a></li>");
        }
    });
}
...
function viewAllCities(i) {
    ...
    $.each(allCities,
        function(i, pname) {
            //if (pname.provinceId == allProId) { //可打开所有城市
            //    allCitys[j++] = pname;
            //}
            if(pname.id == 'uIMcazYzTxeHdTDr+k7rilC+DzM='){
                allCitys[j++] = pname;
            }
        });
